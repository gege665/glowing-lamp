...
describe('resolveReplyStyles', () => {
  ...
  it('仅初识 → 日常社交 14 风格', () => {
    const settings = { ...DEFAULT_SETTINGS, relationshipStages: ['初识'] };
  ...
  it('热恋 → 暧昧私信风格池', () => {
    const settings = { ...DEFAULT_SETTINGS, relationshipStages: ['热恋'] };
...
describe('pickStylesForGeneration', () => {
  it('暧昧模式选取 4 条精准话术', () => {
    ...
    expect(picked.map((s) => s.label)).toEqual(['自然', '暧昧', '高冷', '奶狗']);
  });
  ...
  it('场景优先排序核心风格', () => {
    ...
    expect(picked.map((s) => s.label).slice(0, 3)).toEqual(['自然', '痞帅', '高冷']);
    expect(picked).toHaveLength(4);
  });
  ...
  it('主风格锁定 → 同人设变体', () => {
    const picked = pickStylesForGeneration(FLIRT_REPLY_STYLES, {
      chatScene: 'first_add',
      primaryReplyStyle: 'cool',
    });
    expect(picked).toHaveLength(4);
    expect(picked.every((s) => s.label === '高冷')).toBe(true);
  });

  it('日常模式也输出 4 条核心风格', () => {
    const picked = pickStylesForGeneration(DAILY_REPLY_STYLES);
    expect(picked).toHaveLength(4);
    expect(picked[0].label).toBe('自然');
  });
});
...
describe('resolveReplyGroupHeaders', () => {
  it('分组标题与风格模式一致', () => {
    ...
    expect(
      resolveReplyGroupHeaders(
        pickStylesForGeneration(FLIRT_REPLY_STYLES, {
          chatScene: '',
...
describe('buildBatchStylePromptBlock', () => {
  it('包含风格标签与差异化铁律', () => {
    ...
    expect(block).toContain('差异化铁律');
    expect(block).toContain('禁止换皮同句');
