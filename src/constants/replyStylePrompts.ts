...
/** 暧昧话术质感标杆（仅 prompt 参考，输出须接对方原话改写，禁止照抄） */
export const FLIRT_REPLY_QUALITY_SAMPLES: Record<string, string> = {
  温柔撒娇: '小姐姐～我刷了好多条你的视频，才忍不住来打个招呼～',
  俏皮可爱: '仙女姐姐～陌生人就不能夸你的视频好看吗～',
  高冷撩人: '抱歉，路过，被你视频捶到了。',
  温柔安抚: '别紧张，我就是想认识你，没恶意。',
  暧昧拉扯: '看来我来早了，那先不打扰你。',
  直球进攻: '我是那个想追你的男生，来认识一下。',
  幽默调侃: '我是你未来的朋友，提前来报个到。',
  成熟稳重: '抱歉打扰，我是做自媒体的，觉得你视频不错。',
  欲擒故纵: '你凶我干嘛，我又没说要赖着不走。',
  温柔体贴: '别怕，我就正常认识你，真没别的意思。',
  小坏痞帅: '这就查我户口啊？行吧，我老实交代。',
  文艺深情: '刷到你的视频好多次，忍不住想来打个招呼。',
  轻松随意: '你可能不认识我，但我挺有趣的。',
  神秘诱惑: '你不认识我没关系，我认识你就够了。',
};
...
const FLIRT_STAGES = new Set(['暧昧阶段', '升温阶段', '亲密阶段', '确定关系']);

/** 5 条精准话术分组（话术区 UI） */
export const CORE_EIGHT_GROUP_HEADERS = [
  { index: 0, title: '精准首选', subtitle: '场景推荐 · 3 条' },
  { index: 3, title: '补充表达', subtitle: '备选 · 2 条' },
] as const;

/** 单次分析实际生成的话术条数（5 条精准接话） */
export const GENERATION_REPLY_COUNT = 5;
...
export function resolveReplyStyles(
  ...
  const onlyFirstMeet =
    stages.length > 0 && stages.every((s) => s === '初识阶段');
  if (onlyFirstMeet) {
...
export function buildBatchStylePromptBlock(styles: ReplyStyleDefinition[]): string {
  const styleLines = styles
    .map((s, i) => `${i + 1}.【${s.label}】${s.brief} — 须正面回应她最后一句`)
    .join('\n');
  return `【${styles.length} 条精准话术 · 每条对应一种风格】
${styleLines}

【精准铁律】
- 先答她话里的问题/情绪，再带态度；禁止答非所问
- 禁止套话：刷到你抖音/视频/动态、摄影爱好者、咨询小问题、加个微信慢慢说
- 禁止编造她没提过的事（探店/合作/店铺等）
- 每条开头词不同，气质符合【】内风格

【输出】恰好 ${styles.length} 行，每行 ONLY 一句 15～35 字口语，无编号、无【】标签、无解释`;
}

export const REPLY_BATCH_OUTPUT_RULES = `【批次输出铁律 · 5 条精准话术】
- 恰好输出对应行数，每行 ONLY 一句可复制发送的口语原话
- 每条必须精准回应对方最后一句的核心意图，禁止答非所问、禁止套话
- 严禁整行出现：例如、比如、表明来意、注意每条、风格名、第N条、思考过程
- ${REPLY_LENGTH_RULE}
- ${REPLY_THINKING_BAN}`;

export const REPLY_STYLE_MODE_HINT =
  '根据对方消息生成 5 条精准话术。8 种风格池按场景优选，可点隐形提示词微调语气。';

/** Header / 设置页 · 话术风格说明 */
export const REPLY_STYLE_HEADER_TAGLINE =
  'AI高情商回复 · 5条精准话术 · 截图识别 · 反捞守护';

export const REPLY_STYLE_SETTINGS_SUMMARY = `豆包 Mini 生成 14 条私信原话（仅可复制发送的句子，无说明无思考）：
· 暧昧私信：温柔撒娇、俏皮可爱、高冷撩人…（默认）
· 日常社交：礼貌友好、轻松随和…（仅初识阶段）
· 越界专项：俏皮拉扯、幽默划界…（检测到越界消息）`;
