...
export function buildScenarioReplyGuidance(scenario: SocialScenario): string {
  return `【识别场景】${scenario.label}
【心理焦点】${scenario.psychFocus}
【回复原则】${scenario.commPrinciples.join('；')}
【宜】${scenario.replyDos.join('、')}
【忌】${scenario.replyDonts.join('、')}
【范例语气参考（勿照抄，须接对方原话改写）】${scenario.examples.join(' / ')}`;
}
