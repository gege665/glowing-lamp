...
export function hasMultipleSelectableModels(provider: AIProvider): boolean {
  return getUniqueSelectableModels(provider).length > 1;
}

/** @deprecated 仅文档用途；请以 getSelectableModels(provider) 校验模型是否可用 */
export const LEGACY_FOREIGN_MODELS = new Set([
...
export function getModelLabelForTask(task: ModelTask): string {
  return MODEL_TASK_LABELS[task];
}
