...
import StrategyCoachSection from './StrategyCoachSection';
...
export default function AnalysisPanel() {
  ...
  if (isAnalyzing) {
    ...
    return (
      <div className="panel-card h-full flex flex-col overflow-hidden">
        <div className="px-4 py-3 border-b border-soul-700/30 flex items-center gap-2 shrink-0">
          <Loader2 className="w-5 h-5 text-soul-400 animate-spin" />
          <h2 className="font-semibold text-soul-200">心理分析中</h2>
        </div>
        <div ref={streamRef} className="flex-1 overflow-y-auto scroll-touch p-4 min-h-0 flex flex-col gap-3">
          ...
          {streamProgress.phase === 'replies' || streamProgress.phase === 'finishing' ? (
            <p className="text-center text-soul-500 text-xs">
              ...
              <span className="lg:hidden">分析完成，请切换到「话术」查看生成进度</span>
            </p>
          ) : null}
        </div>
      </div>
  ...
  if (!analysis) {
    ...
    return (
      ...
      <div className="panel-card h-full flex flex-col items-center justify-center gap-4 p-6 overflow-y-auto">

        <Brain className="w-12 h-12 text-soul-500/40 shrink-0" />

        <div className="text-center max-w-xs space-y-2">
          <p className="text-soul-300 font-medium">心理分析</p>
          <p className="text-soul-500 text-sm leading-relaxed">
            {messages.length > 0 ? (
              <>
                <span className="hidden lg:inline">点击左侧「智能生成话术」，分析结果将显示在这里</span>
                <span className="lg:hidden">在「对话」页点击「智能生成话术」，结果将显示在此</span>
              </>
            ) : (
              <>
                <span className="hidden lg:inline">先在左侧粘贴对方消息，再点击「智能生成话术」</span>
                <span className="lg:hidden">先在「对话」页粘贴对方消息，再点击「智能生成话术」</span>
              </>
            )}
          </p>
        </div>
    ...
    <div className="panel-card h-full flex flex-col overflow-hidden">
      ...
      <div className="px-4 py-3 border-b border-soul-700/30 flex items-center justify-between shrink-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <Brain className="w-5 h-5 text-soul-400" />

          <h2 className="font-semibold text-soul-200">心理分析</h2>
      ...
      <div className="flex-1 overflow-y-auto scroll-touch p-3 sm:p-4 space-y-4 min-h-0">
        ...
        {analysis.analysisDegraded && (
          <div className="p-3 rounded-xl border border-amber-500/40 bg-amber-500/10 text-amber-100/90 text-sm leading-relaxed">
            <p className="font-medium text-amber-200">分析未完整</p>
            <p className="mt-1 text-xs text-amber-100/80">
              {analysis.analysisDegradedReason || '当前为本地摘要，话术仅供参考'}
              。建议点击右上角「重新分析」。
            </p>
          </div>
        )}
        ...
        <StrategyCoachSection analysis={analysis} />
        ...
        <Section title="沟通策略" icon={<Target className="w-4 h-4 text-green-400" />}>
          ...
          <div className="space-y-3 text-sm">
            ...
            <StrategyItem title="沟通策略" content={strategy.communicationStrategy} />

            <StrategyItem title="下一步" content={strategy.nextMove} accent />
