...
function GroupHeader({ title, subtitle }: { title: string; subtitle?: string }) {
  return (
    <div className="px-1 py-2 sticky top-0 bg-teal-950/80 backdrop-blur-sm z-10 border-b border-teal-800/30 mb-1">
...
export default function ReplyPanel() {
  ...
  if (isAnalyzing) {
    ...
    return (
      <div className="panel-card panel-zone-reply h-full flex flex-col overflow-hidden">
        <div className="px-4 py-3 border-b border-teal-500/15 shrink-0">
          <div className="flex items-center gap-2">
            <Loader2 className="w-5 h-5 text-teal-400 animate-spin" />
            <h2 className="font-semibold text-soul-200">回复区</h2>
            {isReplyPhase && (
  ...
  if (!analysis || !(analysis.replies?.length > 0)) {
    return (
      <div className="panel-card panel-zone-reply h-full flex flex-col items-center justify-center gap-4 p-6">
        <MessageSquare className="w-14 h-14 text-teal-500/40" />
        <div className="text-center max-w-xs space-y-2">
          <p className="text-soul-300 font-medium">回复区</p>
          <p className="text-soul-500 text-sm leading-relaxed">
            {messages.length > 0
              ? '点击左侧「智能生成话术」，将生成至少 3 条差异化方案，不满意直接看下一条'
              : '粘贴对方消息后，在左侧生成分析与多套差异化话术'}
          </p>
        </div>
      </div>
    );
