...
export default function FemalePsychologySection({ insight }: Props) {
  ...
  const coreNeeds = insight.coreNeeds ?? [];
  const hasInsight =
    ...
    insight.scenarioTip ||
    insight.shitTestNote;

  const scenarioLabel = insight.socialScenario || '';
  const matchedScenario = SOCIAL_SCENARIOS.find(
    (s) => s.label === scenarioLabel || scenarioLabel.includes(s.label)
  );
      ...
      {hasInsight && (
        <div className="p-3 bg-soul-950/30 rounded-xl border border-pink-500/20">
          ...
          <div className="space-y-2 text-sm">
            ...
            {insight.shitTestNote && (
              <InfoRow label="试探 / 废物测试" value={insight.shitTestNote} highlight />
            )}
            {insight.scenarioTip && (
