        ...
        <div className="flex-1 overflow-y-auto scroll-touch p-5 space-y-5 min-h-0">
          ...
          <div className="p-3 bg-green-500/10 border border-green-500/20 rounded-xl flex gap-3">
            ...
            <div className="text-xs text-green-200/80 leading-relaxed">
              ...
              {serverKeyConfigured
                ? '对话历史仅存于本机。若你已在下方填写个人 Key，将优先用本机 Key；留空才用服务端 Key。个人 Key 不会写入 Vercel 环境变量。'
                : '个人自用：API Key 仅保存在本机浏览器 localStorage，不配置到 Vercel。生成时密钥仅转发至你选择的 AI 服务商，服务端不落盘。'}
            </div>
          </div>
          ...
          <SceneStyleSelector
            ...
            primaryReplyStyle={local.primaryReplyStyle ?? ''}
            ...
            onPrimaryStyleChange={(primaryReplyStyle) => setLocal({ ...local, primaryReplyStyle })}
          />
            ...
            <div className="flex items-center justify-between mb-1">
              ...
              <div className="flex gap-2 text-xs">
                ...
                <button
                  type="button"
                  className="text-soul-400 hover:text-pink-300"
                  onClick={() => setLocal({ ...local, relationshipStages: ['暧昧'] })}
                >
                  仅暧昧
                </button>
              </div>
