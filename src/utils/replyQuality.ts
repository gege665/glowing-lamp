...
/** 客套卑微 / 土味油腻 / 查户口压迫（单条命中即不合格） */
export const GREASY_OR_SERVILE_PATTERNS = [
  /不好意思.*打扰/,
  /打扰(你|您)了/,
  /您方便吗/,
  /求(你)?回/,
...
export function lineLooksGreasyOrServile(line: string): boolean {
  return GREASY_OR_SERVILE_PATTERNS.some((p) => p.test(line));
}
...
export function isValidReplyLine(
  ...
  const t = line.trim();
  const intent = lastOther.trim() ? detectMessageIntent(lastOther) : null;
  const identityVerify = Boolean(
    intent && (intent.asksIdentity || intent.asksPurpose || intent.isWary)
  );
  // 与实战铁律 10～20（上限 28）对齐，避免略长口语被整批滤光
  if (t.length < 4 || t.length > 28) return false;
  if (lineLooksLikeReasoningLeak(t)) return false;
  if (lineLooksTooOfficial(t)) return false;
  if (lineLooksGreasyOrServile(t)) return false;
  // 她问你是谁时，「我是…」是正确答法，勿当开场废句滤掉
  if (!identityVerify && lineLooksLikeNameDumpOpening(t)) return false;
  if (options?.openingMode && lineLooksLikeOpeningCringe(t)) return false;
  if (lineLooksOffTopic(t, lastOther)) return false;
  return true;
}

export type ReplyQualityIssue = 'meta' | 'official' | 'offTopic' | 'greasy';
...
export function detectBatchQualityIssues(raw: string, lastOther: string): ReplyQualityIssue[] {
  ...
  const greasyCount = lines.filter(lineLooksGreasyOrServile).length;
  ...
  if (greasyCount >= Math.max(1, Math.ceil(lines.length * 0.2))) issues.push('greasy');
