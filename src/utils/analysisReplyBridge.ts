...
export function buildSourceContextHint(
  ...
  const mp = settings.myProfile ?? { hobbies: '', experiences: '', strengths: '', topics: '' };
...
export function buildAnalysisInsightBlock(
  ...
  const lines = [
    '【左侧心理分析结论 · 5 条话术必须逐条落实，禁止与分析矛盾】',
    line('一句话判断', analysis.summary),
    line('意图标签', analysis.intentLabel),
    line('读心要点', analysis.intentInsight?.slice(0, 80)),
    line('主要情绪', analysis.emotion?.primary),
    line('潜台词', psy.subtext),
    line('她最缺什么', psy.mentalState),
    line('核心需求', needs),
    line('沟通模式', fp.commStyle),
    line('回复原则', fp.replyPrinciple),
    line('场景方向', fp.scenarioTip),
    line('下一步策略', strat.nextMove || strat.communicationStrategy),
    line('框架调整', strat.frameAdjust),
    line(
      '对策示例',
      (analysis.strategyCards ?? [])
        .map((c) => c.example)
        .filter(Boolean)
        .slice(0, 2)
        .join(' ｜ ')
