...
export function buildMessageSpecificDirective(
  ...
  // 仅「真·刚通过/寒暄」走开场；她问你是谁时绝不能用「不是在查身份」
  const isOpeningOnly =
    isFriendApprovalMessage(lastOther) ||
    (chatScene === 'first_add' &&
      !intent.asksIdentity &&
      !intent.asksPurpose &&
      !intent.isWary);
...
【刚加好友 · 第一条 · 最高优先级】对方「${parsed.content || lastOther}」= 已通过/寒暄，不是在查你身份。
公式：好奇 + 细节 + 共鸣（点头像/昵称/氛围，再轻轻靠近）。
硬性规则（违反即失败）：
1. 禁止目的宣告：我是来认识你的 / 想认真聊聊 / 正式认识一下
2. 禁止自我介绍腔：我是刚加你的 / 自报家门 / 我叫XX刚通过 / 刚通过你好友
3. 4 条气质必须明显不同，禁止同义改写
4. 未问身份不要报名字；10～20 字；不油不跪不查户口
5. 每条要有具体落点（夸细节 / 轻悬念 / 轻钩子 三选一）
...
【出现即失败】
通过了，我是来认识你的 / 我是刚加你的，想认真聊聊 / 你一问我先自报家门了 / 嗨我是刚加你的那位 / 我，刚通过你好友`;
  }

  if (intent.asksIdentity) {
    must.push(
      '她问了「你是谁」→ 每条须先简明自报身份+来源（我是谁/怎么加上的），再带轻钩子；禁止只回「嗯然后呢/哈哈」糊弄'
    );
  }
  if (intent.asksPurpose) {
    must.push(
      '她问了「什么事」→ 每条须直接说来意/找她什么事，禁止「没事/随便聊聊/就想打招呼/嗯然后呢」糊弄'
    );
  }
